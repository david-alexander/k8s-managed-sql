...
AS
BEGIN
	DECLARE @Sql NVARCHAR(MAX) = (SELECT 'BACKUP LOG [' + ORIGINAL_LOGIN() + '] TO URL = N''__BACKUPS_AZURE_BLOB_CONTAINER_URL__/' + ORIGINAL_LOGIN() + '-TransactionLog-' + CONVERT(NVARCHAR(MAX), GETUTCDATE(), 126) + 'Z.bak'' WITH  COPY_ONLY;');
	EXEC(@Sql);
END
GO
